...
class MediaCategorySelect(discord.ui.Select):
    ...
    def __init__(self, *args, user: User, media_categories: list[MediaCategory], **kwargs):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        modal = self.modal_class(user=self.user, media_category=media_category)
        await interaction.response.send_modal(modal)


class MediaCategorySelectView(BaseView):
    select_class = MediaCategorySelect

    def __init__(self, *args, user: User, media_categories: list[MediaCategory], **kwargs):
        super().__init__(*args, **kwargs)
        self.add_item(self.select_class(user=user, media_categories=media_categories))
